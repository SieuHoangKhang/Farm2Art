  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardBody>
            <p className="text-sm text-stone-500 mb-2">Tổng doanh thu</p>
            <p className="text-3xl font-bold text-emerald-600">
              {formatCurrency(data.totalRevenue)}
            </p>
            <p className="text-xs text-stone-400 mt-2">VNĐ</p>
          </CardBody>
        </Card>

        <Card>
          <CardBody>
            <p className="text-sm text-stone-500 mb-2">Tổng đơn hàng</p>
            <p className="text-3xl font-bold text-emerald-600">{data.totalOrders}</p>
            <p className="text-xs text-stone-400 mt-2">↑ 12% từ tháng trước</p>
          </CardBody>
        </Card>

        <Card>
          <CardBody>
            <p className="text-sm text-stone-500 mb-2">Người dùng</p>
            <p className="text-3xl font-bold text-purple-600">{data.totalUsers}</p>
            <p className="text-xs text-stone-400 mt-2">↑ 18% từ tháng trước</p>
          </CardBody>
        </Card>

        <Card>
          <CardBody>
            <p className="text-sm text-stone-500 mb-2">Sản phẩm hoạt động</p>
            <p className="text-3xl font-bold text-orange-600">{data.activeListings}</p>
            <p className="text-xs text-stone-400 mt-2">↑ 5% từ tháng trước</p>
          </CardBody>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card>
          <CardBody>
            <h3 className="text-lg font-semibold text-stone-800 mb-6">Doanh thu tuần</h3>
            <div className="space-y-3">
              {data.weeklyRevenue.map((item, idx) => (
                <div key={idx}>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm font-medium text-stone-600">{item.day}</span>
        ...
        <Card>
          <CardBody>
            <h3 className="text-lg font-semibold text-stone-800 mb-6">Top 5 người bán</h3>
            <div className="space-y-3">
              {data.topSellers.map((seller, idx) => (
                <div key={idx} className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-stone-800">#{idx + 1}</p>
                    <p className="text-sm text-stone-500">{seller.name}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-stone-800 w-12">
                      {seller.sales}
                    </span>
                  </div>
                </div>
      ...
      <Card>
        <CardBody>
          <h3 className="text-lg font-semibold text-stone-800 mb-6">Tăng trưởng người dùng</h3>
          <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
            {data.userGrowth.map((item, idx) => (
              <div key={idx} className="text-center">
                ...
                <p className="text-xs text-stone-500">{item.month}</p>
                <p className="text-sm font-semibold text-stone-800">{item.count}</p>
              </div>
            ))}
          </div>
        </CardBody>
